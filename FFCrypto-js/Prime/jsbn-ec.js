...
// -1- original library with some small forkings (Basic JavaScript BN library -
// subset useful for RSA encryption)
// -2- some useful additional functions including karatsuba multiplication
// -3- some efficient functions (reduction-addition-subtraction) when working in
// Z_{p28} with p28 = 2^224 + 2^140 + 2^56 + 1
...
function bnpClamp() {
	...
	while (this.t > 0 && this.arr[this.t - 1] == c)
		--this.t;
}
...
function bnCompareTo(a) {
	...
	if (r != 0) return r;
	while (--i >= 0)
		if ((r = this.arr[i] - a.arr[i]) != 0) return r;
	return 0;
}
...
function bnpSquareTo(r) {
	...
	for (i = 0; i < x.t - 1; ++i) {
		...
		if ((r.arr[i + x.t] += x.am(i + 1, 2 * x.arr[i], r, 2 * i + 1, c, x.t
				- i - 1)) >= x.DV) {
			r.arr[i + x.t] -= x.DV;
			r.arr[i + x.t + 1] = 1;
		}
...
function bnpDivRemTo(m, q, r) {
	...
	while (--j >= 0) {
		...
		var qd = (r.arr[--i] == y0) ? this.DM : Math.floor(r.arr[i] * d1
				+ (r.arr[i - 1] + e) * d2);
...
function montReduce(x) {
	...
	for ( var i = 0; i < this.m.t; ++i) {
		...
		var u0 = (j * this.mpl + (((j * this.mph + (x.arr[i] >> 15) * this.mpl) & this.um) << 15))
				& x.DM;
...
// -----------------------------------------------------------------------------------------
//
function bnRightShift(n) {

	r = nbi();
	this.rShiftTo(n, r);
	return r;
}
function bnLeftShift(n) {

	r = nbi();
	this.lShiftTo(n, r);
	return r;
}
function bnSquareMod(m) {
...
// ----------------------------------------------------------------------2/3-------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------
// This second part contains some useful functions added to the initial jsbn
// library
//
...
function randBigInt(sizeInteger) {
	...
	var xx = new String('');
	for ( var index = 0; index < sizeInteger; index++) {
...
function bnIsZero() {
	...
	var i = this.t;
	while (i--) {
		if (this.arr[i] !== 0) return 0;
	}
	return 1;
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------
// this function multiplies a big integer with a small integer < 8
// multIntTo calls bnpMultIntTo2 but it can be changed at the end of the library
// (BigInteger.prototype.multIntTo=bnpMultIntTo)
function bnMultInt(n) {

	var r = nbv(0);
	return this.multIntTo(n, r);
}
...
// this function multiplies by a small integer by "shifting and adding" the big
// integer according to small integer bit representation
function bnpMultIntTo(n, r) {

	var count = 0;
	while (n > 0) {
		if (n & 1) {
			r = r.add(this.shiftLeft(count));
		}
		count++;
		n = n >> 1;
	}
...
// Depending on the browser, computer, OS, ... cross-over may happen between 600
// and 3000 bits
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------
// These functions reshapes a big integer. If a function outputs a big integer
// which array elements are larger than 2^28, use these functions
// to reshape the result.
// this.voidresh() reshapes this while this.reshape() returns a reshaped copy of
// this without modifying this
...
// ----------------------------------------------------------------------------------3/3-------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------
// This third pard contains functions that have been implemented to accelerate
// computations in Z_{p28}, where p28 = 2^224 + 2^140 + 2^56 +1 is a prime
// that has been chosen for the particular architecture of the library that
// represents big integers with arrays which elements are 28 bits long.
// The functions require entries in Z_{p28} and outputs the result in this field
// too
//
...
// this <= 16 words
// the function modp28 calls the private function pModp28 that accepts only
// entries between 0 and p
function bnModp28(m) {
...
function bnpModp28(m, r) {
	...
	// to overflow (>28 bits), c being a very small integer (most of the time 0
	// or 1).
	// ---------------------------------------------------------------
	if (r.arr[7] >> 28) {
		c = r.arr[7] >> 28;
...
function bnpSubTo2(b, r) {
	...
	var a, c, c7, i;
	for (i = b.t; i < 8; i++)
		b.arr[i] = 0; // fill the array with 0's to avoid operations with
	// undefined numbers
	for (i = this.t; i < 8; i++)
		this.arr[i] = 0; // the same with this
	...
	if (r.arr[7] >> 28) {
		r = nbi();
		this.subTo(b, r);
	} // almost never happens because c is a very small integer
...
// these functions allow to carry out operations without need to create a new
// object,
// which may take some time, when an already instanciated big integer can be
// re-used
// this has been used for addition and doubling of elliptic curve points
BigInteger.prototype.spAdd = function(b, r) {

	this.addTo2(b, r);
	return r;
};
BigInteger.prototype.spSubtract = function(b, r) {

	this.subTo2(b, r);
	return r;
};
BigInteger.prototype.spMod = function(m, r) {
